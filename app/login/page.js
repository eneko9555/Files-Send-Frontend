...
const page = () => {
  ...
  return (
    <div className="md:w-3/4 lg:w-1/2 m-5 md:m-auto">
      <h2 className="text-4xl font-bold mt-24 text-center mb-8">Iniciar Sesión</h2>
      <form className=" bg-white shadow-md py-8 rounded-md p-5" onSubmit={formik.handleSubmit}>
        ...
        <button type="submit" className="w-full bg-cyan-700 p-3 text-white font-semibold uppercase rounded-md hover:opacity-70 hover:cursor-pointer duration-300" > {loading ? <PuffLoader size={20} color="white" loading={loading} /> : "Iniciar Sesión"}</button>
      </form>
    </div>
  )
}
